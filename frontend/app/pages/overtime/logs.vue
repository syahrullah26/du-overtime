...
import { getActionColor } from "~/utils/helper";
...
const fetchLogs = async (page = 1) => {
  ...
  try {
    const response = await useApiFetch<PaginatedResponse<OvertimeLog>>(
      `/overtime-logs?page=${page}`,
    );
  ...
  <div class="min-h-screen bg-[var(--white-bone)] p-8">
    ...
    >
      ...
      <div v-else class="overflow-x-auto">
        <table class="w-full text-left">
          ...
          <tbody class="divide-y divide-gray-50">
            ...
            >
              ...
              <td class="px-6 py-4">
                <div class="flex flex-col">
                  <span class="text-sm font-semibold text-gray-800">{{
                    log.action_by_user?.name || log.action_by
                  }}</span>
                  <span class="text-xs text-gray-500">{{
                    log.action_by_user?.role
                  }}</span>
                </div>
              </td>
              <td class="px-6 py-4">
              ...
              <td class="px-6 py-4">
                <div
                  class="flex items-center gap-2 text-xs truncate max-w-[200px]"
                >
                  <span v-if="log.old_status" class="text-gray-400">{{
                    log.old_status
                  }}</span>
        ...
        <!-- Pagination -->
        <div
          class="p-6 bg-gray-50 border-t border-gray-100 flex justify-center gap-2"
        >
          <button
